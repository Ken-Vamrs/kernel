...
static int goodix_ts_power_init(struct goodix_ts_core *core_data)
{
	...
	if (ts_bdata->avdd_name) {
		...
		/* SCG ... Only set voltage if regulator supports it (not a fixed regulator) */
		/*if (regulator_count_voltages(core_data->avdd) > 0) {
			r = regulator_set_voltage(core_data->avdd, 2800000, 3300000);
			if (r) {
				ts_err("regulator_set_voltage failed %d\n", r);
				return r;
			}
		} else {
			ts_info("Fixed regulator detected, skipping voltage set");
		}
		*/	
	} else {
