...
static int goodix_close_hidi2c_mode(struct goodix_ts_device *ts_dev)
{
	...
	if (try_times >= 10) {
		ts_info("%s FAILED,0x%04x is not equal to 0xff", __func__, ts_dev->reg.command);
		return -EINVAL;
	}
	...
	for (try_times = 0; try_times < 3; try_times++) {
		if (ts_cmd.initialized) {
			...
			/*read command register, if it's not 0xFF,continue*/
			for (j = 0; j < 3; j++) {
				if (goodix_i2c_read(ts_dev, ts_dev->reg.command, buffer, 1)
				!= 0)
					continue;
				else {
					if (buffer[0] != 0xFF) {
						ts_info("try_times:%d:%d,"
						TAG_I2C " read 0x%04x:0x%02x",
							try_times,
							j, ts_dev->reg.command, buffer[0]);
						usleep_range(10000, 11000);
						continue;
					} else
						goto exit;
		...
		u8 *pre_buf, u32 pre_buf_len)
{
	...
	/* CHECKSUM VALIDATION DISABLED - This GT9886 variant doesn't use checksums or uses different validation */
	if (dev->ic_type == IC_TYPE_NANJING) {
	...
	/* Checksum check disabled - coordinate data is valid even though checksums don't match
	...
	for (; retry > 0; retry--) {
		...
		if (dev->ic_type == IC_TYPE_NORMANDY)
			...
			if (i >= 10) {
				...
				TAG_I2C " 0x8000, command register not equal 0xff");
				continue;
			}
			/*3. close hid i2c*/
			goodix_close_hidi2c_mode(dev);
