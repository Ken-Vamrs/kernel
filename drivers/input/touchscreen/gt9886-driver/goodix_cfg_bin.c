...
/* Hardcoded surrogate config package with register addresses from GT9886 Programming Guide V0.1 */
static struct goodix_cfg_package gt9886_surrogate_cfg = {
	.cnst_info = {
		.pkg_len = sizeof(struct goodix_cfg_pkg_const_info) + sizeof(struct goodix_cfg_pkg_reg_info),
		.ic_type = "nanjing",  /* GT9886 is a nanjing chip */
		.cfg_type = TS_NORMAL_CFG,  /* 0x01 - this is TS_NORMAL_CFG from stock header */
		.sensor_id = 0,  /* sensor 0 - the only touchscreen on the system */
		.hw_pid = "9886",  /* Expected Product ID from chip (chip returns "9886" not "GT9886") */
		.hw_vid = {0},
		.fw_mask = {0},
		.fw_patch = {0},
		.x_res_offset = 0,
	...
	.reg_info = {
		/* Note: reserved1 = LENGTH for version/pid/vid, MASK for sensor_id; reserved2 is unused */
		/* Stock header had 9 TS_REG defines, some don't map to cfg_package fields (DOZE_CTRL 0x30F0, DOZE_STAT 0x3100) */
		.cfg_send_flag = {.addr = 0x0000, .reserved1 = 0, .reserved2 = 0},  /* unknown - no stock define */
		.version_base = {.addr = 0x4532, .reserved1 = 4, .reserved2 = 0},  /* FW version base - Programming Guide page 5, 4 bytes (was TS_REG_VERSION 0x8240 in stock header) */
		.pid = {.addr = 0x4535, .reserved1 = 6, .reserved2 = 0},  /* Product ID - Programming Guide page 5, 6 bytes (was TS_REG_PID 0x4535 - MATCHES stock header!) */
		.vid = {.addr = 0x453E, .reserved1 = 3, .reserved2 = 0},  /* Patch FW Version - Programming Guide page 5, 3 bytes (0x453E-0x4540) */
		.sensor_id = {.addr = 0x4541, .reserved1 = 0xFF, .reserved2 = 0},  /* Sensor ID - Programming Guide page 5, mask 0xFF, expect sensor 0 */
		.fw_mask = {.addr = 0x0000, .reserved1 = 0, .reserved2 = 0},  /* unknown - no stock define */
		.fw_status = {.addr = 0x6F6F, .reserved1 = 0, .reserved2 = 0},  /* FW Status - Programming Guide page 6, no stock define */
		.cfg_addr = {.addr = 0x8050, .reserved1 = 0, .reserved2 = 0},  /* Config base address (was TS_REG_CFG_BASE 0x8050 in stock header) */
		.esd = {.addr = 0x3103, .reserved1 = 0, .reserved2 = 0},  /* ESD - Programming Guide page 8 (was TS_REG_ESD_TICK_R 0x3103 - MATCHES stock header!) */
		.command = {.addr = 0x6F68, .reserved1 = 0, .reserved2 = 0},  /* Command - Programming Guide page 6 (was TS_REG_CMD 0x8040 in stock header) - FIXED hardcoded 0x8040 refs in goodix_ts_i2c.c:1377,1388,1399 to use ts_dev->reg.command */
		.coor = {.addr = 0x4100, .reserved1 = 0, .reserved2 = 0},  /* Coordinates - Programming Guide page 4 (was TS_REG_COORDS_BASE 0x824E in stock header) */
		.gesture = {.addr = 0x0000, .reserved1 = 0, .reserved2 = 0},  /* unknown - no stock define */
		.fw_request = {.addr = 0x6F6D, .reserved1 = 0, .reserved2 = 0},  /* FW Request - Programming Guide page 6 (was TS_REG_REQUEST 0x8044 in stock header) */
		.proximity = {.addr = 0x0000, .reserved1 = 0, .reserved2 = 0},  /* unknown - no stock define */
		.reserved = {0},
	},
	.cfg = NULL,  /* No additional config data */
	.pkg_len = sizeof(struct goodix_cfg_pkg_const_info) + sizeof(struct goodix_cfg_pkg_reg_info),
};
...
int goodix_cfg_bin_proc(void *data)
{
	...
	if (r < 0) {
		ts_err("read cfg_bin from /etc/firmware FAILED, using hardcoded surrogate config");
		/* Use hardcoded surrogate config package instead of loading from file */
		cfg_bin->cfg_pkgs = kzalloc(sizeof(struct goodix_cfg_package), GFP_KERNEL);
		if (!cfg_bin->cfg_pkgs) {
			ts_err("Failed to alloc memory for surrogate cfg_pkgs");
			r = -ENOMEM;
			goto exit;
		}
		/* Copy our surrogate package */
		memcpy(cfg_bin->cfg_pkgs, &gt9886_surrogate_cfg, sizeof(struct goodix_cfg_package));
		cfg_bin->head.pkg_num = 1;
		ts_info("Loaded surrogate config package with Programming Guide register addresses");
	} else {
		/*parse cfg bin*/
